...
def upgrade(request):
    ...
    if request.method == 'POST':
        ...
        print('current plan before setting new plan:', request.user.plan)
        ...
        if current_user.plan == '7days':
            if upgraded_to == '14days':
                current_date.date_of_expiration = current_date. date_of_expiration + timedelta(days=7)
                current_date.save()
            else:
                current_date.date_of_expiration = current_date. date_of_expiration + timedelta(days=14)
                current_date.save()
